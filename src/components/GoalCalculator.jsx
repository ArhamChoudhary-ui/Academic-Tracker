  ...
  if (subjects.length === 0) {
    return (
      <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-6">
        <div className="bg-gradient-to-br from-blue-700 to-blue-800 border border-white/20 rounded-2xl max-w-md w-full shadow-2xl p-8 text-center space-y-4">
          <Target size={40} className="mx-auto text-white/30" />
          <p className="text-xl font-bold text-white">No Subjects Found</p>
          <p className="text-white/60 text-sm">Add subjects to your dashboard first to use the Goal Calculator.</p>
          <button onClick={onClose} className="px-6 py-2.5 bg-white/20 hover:bg-white/30 text-white font-semibold rounded-lg transition-colors">
            Close
          </button>
        </div>
      </div>
